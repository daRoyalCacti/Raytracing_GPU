...
class perlin {
	...
	__device__ static float perlin_interp(const vec3 c[2][2][2], const float u, const float v, const float w) {
	...
	__device__ float turb(const point3& p, const int depth=7) const {
		auto accum = 0.0;
		auto temp_p = p;
		auto weight = 1.0;
