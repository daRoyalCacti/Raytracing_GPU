...
struct texture {
	__device__ virtual color value(const float u, const float v, const point3& p) const = 0;
};
...
struct checker_texture : public texture {
	...
	__device__ checker_texture(const texture *_even, const texture *_odd) : even(_even), odd(_odd) {}
	__device__ checker_texture(const color c1, const color c2) : even(solid_color(c1)), odd(solid_color(c2)) {}
...
__device__ struct noise_texture : public texture {
	perlin noise;
	float scale;	//how detailed the noise is, bigger number := more noise

	noise_texture(const float sc = 1.0) : scale(sc) {}

	virtual color value(const float u, const float v, const point3& p) const override {
...
struct turbulent_texture : public texture {
	...
	turbulent_texture(const float sc = 1.0, const int dpt = 7) : scale(sc), depth(dpt) {}
...
struct marble_texture : public texture {
	...
	marble_texture(const float sc = 1.0) : scale(sc) {}
...
class image_texture : public texture {
	...
	virtual color value(const float u, const float v, const vec3& p) const override {
